...
def load_simulation_data(simulation_params = {'p':100,'q':40, 'r':20, 'b':-10, 't':0}, n1=500, n2=200, svm=False, 
                       random_state=55, intercept=False, train_frac = 0.7):
    
    Xz,y = DataGenerator(**simulation_params).generate(n1, n2,random_state)
    X = Xz[:,1:]
    Z = Xz[:,0].reshape(-1,1)
    if svm:
        y = y*2-1
...
def load_adult_data(filepath = '../data/adult_proc.csv',svm=False,random_state=42, 
                    intercept=False,sensitive_attrs=None, train_frac = 0.7):
...
def load_compas_data(filepath = '../data/compas_proc.csv',svm=False,random_state=42, 
                     intercept=False, sensitive_attrs = None, train_frac = 0.7):
    """
...
def load_german_data(filepath='../data/german.data-numeric', svm =False,random_state=42, 
                     intercept = False, train_frac = 0.7):
    """
...
def load_bank_data(filepath = '../data/bank-full.csv',load_data_size=None,svm=False,random_state=42, 
                   intercept = False, train_frac = 0.7):
...
def load_adult_data_prev(load_data_size=None,svm = False,random_state=42, intercept = False, train_frac = 0.7):
